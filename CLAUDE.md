...
POST /auth/password                    # 改密码(2026-08-23 用户定并实施):带令牌调用
  { "old_password", "new_password"(最短6位) } → { "status", "revoked_devices" }
  旧密码即身份证明(不接邮箱/短信通道);旧密码不对回 403"旧密码不对",
  连错 5 次锁 5 分钟(同登录口径,按用户计)。改完删该用户**其他**全部令牌、
  保留当前这枚(改个密码还要重新登录是白受罪),回执带被踢下线的设备数。
  **找回密码永不做**:需要外部通道并倒逼注册补收邮箱,忘了照旧管理员跑脚本重置。
POST /auth/logout-others               # 退出其他设备(2026-08-23 用户定并实施):带令牌调用
  → { "status", "revoked_devices" }    # 删该用户其他全部令牌,当前这枚保留(点一下把自己
                                       # 也踢了没意义);本来就只有一台时回 0,不算失败。
                                       # 令牌存库天生可吊销(删行即失效),永不过期不是漏洞,
                                       # 缺的是吊销手段与用户自助入口,这个接口即该入口

登录/注册限速(2026-08-23 用户定并实施):两条线各自计数,任一超限即 429
  (带 Retry-After 头,文案"尝试太多次了,请 N 分钟后再试")。
  按用户名:连续失败 5 次 → 该用户名锁 5 分钟,挡"死磕一个账号";
  按来源 IP:每 15 分钟最多 20 次失败,挡"一个来源横扫多个账号";
  注册按来源 IP:每小时最多 10 次邀请码猜测(邀请码同属可暴力猜的秘密)。
  失败才计数、成功即清零;锁定期内直接拒、不再核验密码(故不滚雪球)。
  计数放进程内存(单容器单进程够用;业界标准是 Redis,多实例才需要),
  重启清零可接受。来源 IP 取反向代理转发的第一个地址,无则用直连地址。
