...
- 前端多端分阶段(2026-07-06 用户修正定位):**设计从 iOS app 出发**,Web 是该设计的快速实现(轻量工具定位,先行验证、发链接即用)→ **iOS 原生(Swift + SwiftUI)为主产品** → 安卓原生远期(有真实安卓用户再评估)。**Web 只按手机宽度设计,不做 PC 适配**(PC 打开即居中窄栏);**前端一律用图标库,不用 emoji**。各端独立目录、独立代码,界面代码不跨端复用,后端契约对所有端通用;Web 具体功能形态待与用户对齐。**Web 技术栈(2026-07-06 定)**:React + Vite + Tailwind 自绘组件(按 iOS 观感,不用现成移动端组件库),图标 Lucide,图表 ECharts。**远期构想入档**:iOS 本地版(数据只存设备、无用户概念、自带模型密钥)是云端版之外的第二形态,iOS 阶段再评估;后端换 SQLite 只是"服务器不装 PG"的自托管便利,不等于数据在手机本地,勿混淆
- 这是一个新项目,按下方"当前阶段"推进
