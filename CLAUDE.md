...
十一张表:
- `users`:昵称、身高、性别、出生年份(修正 MET 公式用;体重不放这里)+ password_hash(bcrypt;2026-07-12 契约加,可空过渡——登录实施前存量用户无密码,实施时脚本补)
