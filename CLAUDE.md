...
1. **LLM 只做三件事:解析、名称映射、拆解。绝不做算术,绝不输出建议性文字。**
   ...
   - **菜层与克重守恒**(2026-07-08 用户定):复合菜由 LLM 拆解为"一道菜"结构(菜名+总克重+成分清单,每成分带原话叫法/标准名/克数/形态);硬校验:成分克数之和=总克重(容差±5%),超差带差额打回重拆,重试不过整菜降级为估算一条标 `llm_estimated`。成分逐条入 raw、带同名菜名标签(food_records.dish);**菜不是实体**,合计展示时现算,不建菜表
...
2. **解析优先级(严格顺序):**
   ...
   - **补油补调味显式原则**(2026-07-02 定油,2026-07-08 扩调味):炒/煎/炸没报油 → 自动补"菜籽油约10g"独立记录;糖/芝麻酱/沙拉酱/蚝油等高热量调味按做法常识自动补条(盐/醋/生抽忽略);均标 `llm_estimated`、可删可改,绝不隐性折进菜品热量;官方表无调味条目,表外调味必须带估算兜底。用户自己报了则用用户的
...
6. **最小登录制**(2026-07-09 用户定,取代旧"不做 auth"):邀请码注册(码由管理员手工生成)+ 昵称密码登录 + 登录态,三样之外一概不做(无邮箱验证、无找回密码——忘了管理员后台重置、无第三方登录)。实施时全部接口从明报 user_id 改为认登录态。**铁律:登录上线前不把链接发给任何人**(零鉴权期数据库虽在云端,仅限用户本人使用)。排期:设置页之后紧随
...
GET / PATCH /users/{id}                # 设置页·身体档案(2026-07-09):读/改(只发改动字段,
                                       # 昵称重名409);改档案不回算已存记录,读时现算的
                                       # 数字(基础代谢/净摄入)自动按新档案计
GET    /user-foods?user_id=1           # 设置页·自定义食物列表(按更新时间倒序)
DELETE /user-foods/{id}                # 删自定义食物:只影响以后查表,已存记录数字不动;
                                       # 改数值不设接口——对话重复"记住"按名覆盖(user+name 唯一)
GET    /memories?user_id=1             # 设置页·AI 记忆列表(kind: alias|habit|correction)
