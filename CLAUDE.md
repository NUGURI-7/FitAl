...
- 前端多端分阶段(2026-07-06 用户修正定位):**设计从 iOS app 出发**,Web 是该设计的快速实现(轻量工具定位,先行验证、发链接即用)→ **iOS 原生(Swift + SwiftUI)为主产品** → 安卓原生远期(有真实安卓用户再评估)。**Web 只按手机宽度设计,不做 PC 适配**(PC 打开即居中窄栏);**前端一律用图标库,不用 emoji**。各端独立目录、独立代码,界面代码不跨端复用,后端契约对所有端通用;Web 具体功能形态与技术选型待与用户对齐
- 这是一个新项目,按下方"当前阶段"推进
...
2. **算术归代码,判断归 AI**(2026-07-04 用户改定,取代旧"按时间间隔归组"):求和/公式是代码;**归组判断(归哪顿/哪场还是新开)整体交给 AI**——解析调用时附带"当前开放的顿/场摘要+当前时间",AI 逐条判断归属,零额外调用、无任何时间阈值常量。代码只负责执行归属与兜底(无开放组时新开);起名同为 AI 职责
