...
**登录改造总口径(2026-07-12 契约定稿,未实施)**:登录上线时,以上全部业务接口去掉 user_id 参数,改从请求头 `Authorization: Bearer <token>` 认人(查 auth_tokens 表);无令牌/令牌无效一律 401。一刀切不留兼容期,后端/Web/iOS 同批切换。前端双端各做两道守卫:启动时本地无令牌直接进登录页;任何请求收到 401 即清本地令牌、整界面切回登录页。施工顺序:后端(迁移+三接口+全接口改造+单测)→ Web 四件(登录页/请求层带令牌+踢回/设置页退出登录/拆固定用户)→ iOS 同四件;实施需与其他会话的前端工作错开。
