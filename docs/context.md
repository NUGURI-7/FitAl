...
- 已落地:七张表(迁移 0001-0004 均已应用云端 PG)、food.json 1595 条、met.json 80 条、lookup 全部公式、解析器(四级优先级/补油/组间隔计时/表外归档)、AI 归组+起名(无阈值常量,解析时附开放顿/场摘要逐条判断归属并输出组名;AI 没给名按时段兜底,已有名不被兜底覆盖)、AI 记忆("记住"即学:自定义食物/叫法/习惯;全量注入解析 prompt;每日首次请求顺带巩固,失败静默不影响主链路)
...
- 测试 48 个全过(内存 SQLite);演示用户 id=1(昵称"演示",云端库,含若干测试记录与记忆,后续可清)
- 已知解析瑕疵记录在案:"又一组"类表述曾拆两条(已修);牛肉面整体估算未走拆解(可打磨)

**下一步(新会话从这里开始)**
1. 修正/删除接口(PATCH/DELETE,触发聚合增量重算);纠正类记忆(kind=correction)的即时学挂在修正接口上
2. 之后按看板:展示接口(/days /weights /users)→ 真实使用打磨
...
- 2026-07-04:/chat 端点(SSE)+ 时区统一 UTC(修真机 bug);归组由固定阈值改为 AI 判断(用户定);决策入档:不做思考过程展示;AI 起名并入归组调用(时段名兜底);AI 记忆("记住"即学/注入/每日巩固)。两项均真机验证通过。
