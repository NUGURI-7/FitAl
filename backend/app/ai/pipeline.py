...
7. 补充说明不是新记录:同一句里对刚提过的食物追加做法/状态/品牌描述、改口纠正
   ("100克虾仁,水煮的虾仁""不对,150克吧")→ 同一对象仅一条,按最终口径理解;
   只有用户明确说"又吃了/再来一份"才算新记录。
8. 餐次归属:仅当用户明说了这是哪一顿——早餐/早饭→早餐,午餐/午饭/中饭→午餐,
   下午茶/下午加餐→下午茶,晚餐/晚饭→晚餐,夜宵/宵夜→其余——才填 meal_slot;
   没明说必须不填(留空),系统会按时间自动归,绝不要猜。
9. 消息可能附带[用户记忆],解析时优先按记忆理解用户的话。"""
