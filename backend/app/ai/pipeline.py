...
def expand_backfill(
    ...
    - 单组条目明说的自身时长原样保留(如"跑了30分钟"),不参与分摊,
      其占用先从整场里扣除;
    - **多组条目(sets>1)上的时长视为整场口径清空**,参与分摊——几乎无人
      报"每组几分钟",模型把"共练20分钟"填在条目上时,原样保留会让每组
      各继承一份、总时长翻倍(2026-07-11 真机验证抓到);
    ...
    for item in items:
        for _ in range(item.sets or 1):
            ...
            if (item.sets or 1) > 1:
                dup.duration_min = None  # 多组条目的时长=整场口径,交给分摊
            dup.sets = None
            units.append(dup)
