...
async def test_读档案_返回全部档案字段含只读ID(db):
    user = await _user()
    out = await api.get_user(user.id)
    assert out == {
        "id": user.id,
        "nickname": "档案测试",
        "height_cm": 178,
...
async def test_读档案_用户不存在报404(db):
    with pytest.raises(HTTPException) as e:
        await api.get_user(99999)
    assert e.value.status_code == 404


async def test_改档案_只改身高_其余字段不动(db):
    user = await _user()
    out = await api.patch_user(user.id, api.UserPatch(height_cm=180.5))
...
async def test_改档案_可同时改多个字段(db):
    ...
    out = await api.patch_user(
        user.id, api.UserPatch(nickname="新名字", sex="female", birth_year=2000)
    )
...
async def test_改昵称_与他人重名报409(db):
    ...
    with pytest.raises(HTTPException) as e:
        await api.patch_user(user.id, api.UserPatch(nickname="占位者"))
...
async def test_改档案_空请求体报422(db):
    user = await _user()
    with pytest.raises(HTTPException) as e:
        await api.patch_user(user.id, api.UserPatch())
...
async def test_自定义食物列表_按更新时间倒序且只看自己的(db):
    ...
    out = await api.list_user_foods(user.id)
...
async def test_自定义食物列表_用户不存在报404(db):
    with pytest.raises(HTTPException) as e:
        await api.list_user_foods(99999)
    assert e.value.status_code == 404
...
async def test_删自定义食物_记录消失(db):
    ...
    out = await api.delete_user_food(food.id)
...
async def test_删自定义食物_不存在报404(db):
    with pytest.raises(HTTPException) as e:
        await api.delete_user_food(99999)
    assert e.value.status_code == 404
...
async def test_删记忆_记录消失(db):
    ...
    out = await api.delete_memory(memory.id)
