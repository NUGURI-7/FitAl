...
async def test_改克数_表内食物全套重算且餐次总数跟着变(db):
    ...
    card = await api.patch_food(rec.id, api.FoodPatch(grams=100))
...
async def test_直接改热量_采用用户数且来源变自报(db):
    ...
    card = await api.patch_food(rec.id, api.FoodPatch(kcal=300))
...
async def test_改克数_表外记录按比例缩放(db):
    ...
    card = await api.patch_food(rec.id, api.FoodPatch(grams=200))
...
async def test_改普通记录_不产生纠正记忆(db):
    ...
    await api.patch_food(rec.id, api.FoodPatch(grams=100))
...
async def test_运动改时长_按MET重算总耗净耗(db):
    ...
    card = await api.patch_exercise(rec.id, api.ExercisePatch(duration_min=10))
...
async def test_运动直接改热量_净耗按时长扣基础代谢(db):
    ...
    card = await api.patch_exercise(rec.id, api.ExercisePatch(kcal=100))
    assert card["source"] == "user_reported"
